...
class GFRILAdapter(BenchmarkAdapter):
    name = "gfr_il"
    ...
    def run(self, scenario: Scenario, seed: int, output_root: str, python_exec: str) -> Dict[str, Any]:
        ...
        if dataset not in self.SUPPORTED_DATASETS:
            return unsupported_result(self.name, scenario, seed, f"GFR-IL HSI benchmark has no dataset mapping for {dataset}.")
        ...
        try:
            run_command(cmd, cwd=os.path.join(os.getcwd(), "GFR-IL"))
        ...
        if not os.path.exists(result_path):
            return {
                ...
                "seed": int(seed),
                "status": "failed",
                "reason": f"No GFR-IL result.json found under {scenario_root}.",
                "seen_metrics": [],
                "final_oa": None,
                "final_aa": None,
                "final_kappa": None,
