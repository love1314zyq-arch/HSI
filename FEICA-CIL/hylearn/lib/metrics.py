import collections
from hylearn.lib.modules import evaluation
import numpy as np
import torch
...
class MetricLogger:
    ...
    def log_task(self, ypreds, ytrue, task_size, zeroshot=False):
        ...
        OAC, KAPPA, _, _, _, AAC,CAC = evaluation.cluster_accuracy(ytrue.cpu().numpy(), ypreds.cpu().numpy())

        self.metrics["accuracy"].append(
            accuracy_class_split(OAC, CAC,AAC,KAPPA)
        ...
        self._update_accuracy_matrix(self.metrics["accuracy"][-1])
...
#         self._task_counter += 1
    ...
    def _update_accuracy_matrix(self, new_accuracy_per_class):
        for k, v in new_accuracy_per_class.items():
            if k == "total" or k == "avg" or k == "kappa":
                continue
...
def accuracy_class_split(oac, cac,AAC,KAPPA, topk=1):
    """Computes accuracy for the whole test & per task.
    ...
    all_acc["avg"] = AAC
    all_acc["kappa"] = KAPPA    
    for class_id in range(0, len(cac)):
            label = "{}-{}".format(
                str(class_id).rjust(2, "0"),
                str(class_id).rjust(2, "0")
            )
            all_acc[label] =cac[class_id]
...
def forgetting(accuracies):
    ...
    # print("*"*10,usable_tasks)
    forgetting = 0.
    length =0.
    for task in usable_tasks:
        # print("************task",task)
        if task == "total" or task == "avg" or task ==  "kappa":
            # print("************continue",task)
            continue
