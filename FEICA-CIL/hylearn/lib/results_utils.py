...
def get_save_folder(model, date, label, results_root=None):
    if results_root:
        folder_path = os.path.join(results_root, label)
...
def save_results(results, label, model, date, run_id, seed, results_root=None):
    del results["config"]["device"]

    folder_path = get_save_folder(model, date, label, results_root=results_root)
