...
def cluster_accuracy(y_true, y_pre, return_aligned=False):
    ...
    return acc, kappa, nmi, ari, pur, np.mean(ca), ca

def accuracy_calculate(y_true, y_pre):
    # y_best = np.zeros(y_pre.shape)
    Label1 = np.unique(y_true)
    nclass = len(Label1)
    kappa = metrics.cohen_kappa_score(y_true, y_pre)
    ...
    return overall_accuracy*100, average_accuracy,kappa