...
from hylearn.lib.Toolbox.rolling_window import rolling_window as rw
...
class Processor:
    ...
    def get_HSI_patches_rw(self, x, gt, ksize, stride=(1, 1), padding='reflect', is_indix=False, is_labeled=True):
        ...
        pad_needed_height = (new_height - 1) * stride[0] + ksize[0] - x.shape[0]
        pad_needed_width = (new_width - 1) * stride[1] + ksize[1] - x.shape[1]
        # print(pad_needed_height)
        # print(pad_needed_width)
        pad_top = int(pad_needed_height / 2)
        pad_down = int(pad_needed_height - pad_top)
        pad_left = int(pad_needed_width / 2)
        pad_right = int(pad_needed_width - pad_left)
    ...
    def split_each_class(self, X, y, each_train_rate=0.1):
        X_tr, y_tr, X_ts, y_ts = [], [], [], []
        ...
        for c in np.unique(y):
            ...
            each_train_size = math.floor(each_train_rate*len(y_index))
            # each_train_size = round(each_train_rate*len(y_index))
            # each_train_size = 20
        ...
        # for i in range(15):
        #     print(y_tr[i].shape)
        X_tr, X_ts, y_tr, y_ts = np.vstack(X_tr), np.vstack(X_ts), np.concatenate(y_tr,axis=0), np.concatenate(y_ts,axis=0)
        # X_tr, X_ts = np.vstack(X_tr), np.vstack(X_ts)
        # y_tr, y_ts = np.vstack(y_tr), np.vstack(y_ts)
        # X_tr, X_ts, y_tr, y_ts = np.asarray(X_tr), np.asarray(X_ts), np.asarray(y_tr), np.asarray(y_ts)
        # return X_tr.reshape(X_tr.shape[0] * X_tr.shape[1], w,h,c), \
        #        X_ts.reshape(X_ts.shape[0] * X_ts.shape[1], w,h,c), \
