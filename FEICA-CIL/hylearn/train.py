...
import yaml
from hylearn.lib import factory,metrics, results_utils,utils
from hylearn.lib import logger as logger_lib
from hylearn.lib.modules import dpcreate
...
def _train(args, start_date, class_order, run_id):
    ...
    dpblock = _set_dp_model(args)
    inc_dataset, model = _set_data_model(args,dpblock, class_order)
    ...
    for task_id in range(inc_dataset.n_tasks):
        ...
        # ---------------
        model.eval()
        model.before_task(train_loader, val_loader if val_loader else test_loader,args)
    ...
    if args["label"] is not None:
        results_utils.save_results(
            ...
            run_id,
            args["seed"],
            results_root=args.get("results_root")
        )
...
def _set_results(config, start_date):
    if config["label"]:
        results_folder = results_utils.get_save_folder(
            config["model"], start_date, config["label"], results_root=config.get("results_root")
        )
    else:
        results_folder = None
...
def _set_data_model(config,dpblock, class_order):
    inc_dataset = factory.get_data(config, class_order)
    config["classes_order"] = inc_dataset.class_order

    model = factory.get_model(config,dpblock)
    model.inc_dataset = inc_dataset

    return inc_dataset, model
def _set_dp_model(config):
     dpblock = dpcreate.dpcreate(config["channels"])
     return dpblock
...
def _set_seed(seed, nb_threads, no_benchmark, detect_anomaly):
    ...
    torch.backends.cudnn.benchmark = no_benchmark
