...
class IncrementalLearner(abc.ABC):
    ...
    def set_task_info(self, task_info):
        ...
        self._class_state = task_info["class_state"]

    def before_task(self, train_loader, val_loader,args):
        LOGGER.info("Before task")
        self.eval()
        self._before_task(train_loader, val_loader,args)
    ...
    def save_parameters(self, directory, run_id):
        ...
        state = {
            'state': self._old_model.state_dict(),
            'dpblock':self.dpblock,
            # 'linear': net.Conv2d.linear# 将epoch一并保存
        }
