...
class Inc_Learning_Appr:
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        save_root = getattr(self, "save_model_dir", "./model_path")
        os.makedirs(save_root, exist_ok=True)
        save_model_path = os.path.join(save_root, 'task_'+str(t)+'.pth')
    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            self._last_eval_targets = np.asarray(tars, dtype=np.int64)
            self._last_eval_preds_tag = np.asarray(preds_tag, dtype=np.int64)
            self._last_eval_preds_taw = np.asarray(preds_tar, dtype=np.int64)
