...
class HybridHSILiteBackbone(nn.Module):
    def __init__(self, pretrained=False, mode="normal", in_channels=30, feature_dim=512):
        super().__init__()
        _ = pretrained
        self.mode = mode
...
def hybrid_hsi_lite(pretrained=False, mode="normal", **kwargs):
    return HybridHSILiteBackbone(pretrained=pretrained, mode=mode, **kwargs)
